...
if [ -z "$vpc" ]; then
    echo "cluster vpc id is required"
    exit 1
fi

echo "Setup Maestro in ${region} ($vpc)"
...
certs_dir=$ROOT_DIR/_output/aws-certs
source_cert_dir=${certs_dir}/iot/source
policies_dir=$ROOT_DIR/_output/aws-policies
...
echo "$db_pw" > $ROOT_DIR/_output/db.password
...
oc process --filename="https://raw.githubusercontent.com/openshift-online/maestro/refs/heads/main/templates/service-template-rosa.yml" \
    ...
    --param="MQTT_HOST=${mqtt_host}" > ${ROOT_DIR}/_output/maestro-rosa.json

oc -n maestro apply -f ${ROOT_DIR}/_output/maestro-rosa.json
